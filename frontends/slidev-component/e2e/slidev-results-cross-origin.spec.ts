...
// [http://localhost:3030]). The seeded poll IDs are passed to the Slidev deck via
// URL query params (?slug=…&pollId=…&q1Id=…&q2Id=…) on page.goto — no file writes
// needed. The deck reads them from window.location.search. afterAll deletes the poll.
//
...
//   1. Signs in from slide 3 (which reads IDs from URL query params).
...
type SeededData = { slug: string; pollId: string; q1Id: string; q2Id: string };
 ...
 * allowedOrigins already set to the Slidev dev-server origin. Returns the seeded
 * IDs so the caller can pass them as URL query params on page.goto.
 */
...
test.describe("cross-origin slidev deck activation", () => {
  ...
  test("signed-in deck on cross-origin host activates question and renders SSE results", async ({
    ...
    const params = new URLSearchParams({
      slug: seededData.slug,
      pollId: seededData.pollId,
      q1Id: seededData.q1Id,
      q2Id: seededData.q2Id
    });
    await page.goto(`${SLIDEV}/3?${params.toString()}`);
    await expect(page.getByTestId("poll-waiting").first()).toBeVisible({ timeout: 10_000 });

    // Sanity: configureDeckAuthBackend(pollServer) ran via the CustomNavControls
    // headmatter side-effect (reads pollServer from the deck's frontmatter).
